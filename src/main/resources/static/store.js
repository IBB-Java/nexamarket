...
function currentSellerId() { return Number($("#sellerId").value || localStorage.getItem("nexa_seller_id") || state.user?.id || 1); }
function sellerHeaders(sellerId = currentSellerId()) { return {"X-Seller-Id": String(sellerId), Authorization: ""}; }
async function openSellerArea() {
    $("#sellerId").value = String(Number(localStorage.getItem("nexa_seller_id") || state.user?.id || 1));
...
async function ensureCategory(name) {
    ...
    return (await api("/api/v1/categories", {method: "POST", headers: {Authorization: ""}, body: JSON.stringify({name, description: `${name} seçkisi`, parentCategoryId: null})})).id;
}
...
async function createProduct(payload, silent = false) {
    const categoryId = await ensureCategory(payload.category), headers = sellerHeaders(payload.sellerId || 1);
    let product = await api("/api/v1/products", {method: "POST", headers, body: JSON.stringify({name: payload.name, description: payload.description,
    ...
    if (payload.publish !== false) product = await api(`/api/v1/products/${product.id}/publication`, {method: "PATCH", headers, body: JSON.stringify({status: "ACTIVE"})});
...
async function seedCatalog() {
    ...
    try {
        for (const product of demoProducts) { try { await createProduct({...product, sellerId: 1}, true); } catch (error) { if (!error.message.includes("SKU zaten")) throw error; } }
        await loadProducts(); toast("Mağaza keşfe hazır.", "success");
    } catch (error) { toast(error.message, "error"); } finally { setBusy(button, false); }
}
async function submitSeller(event) {
    event.preventDefault(); const sellerId = currentSellerId(), submit = $("#sellerSubmitButton");
    localStorage.setItem("nexa_seller_id", String(sellerId)); $("#sellerMessage").textContent = ""; setBusy(submit, true, "Ürün kaydediliyor");
    try {
        const product = await createProduct({name: $("#sellerProductName").value.trim(), category: $("#sellerCategory").value.trim(), price: Number($("#sellerPrice").value), stock: Number($("#sellerStock").value), sku: $("#sellerSku").value.trim(), sellerId, description: $("#sellerDescription").value.trim() || "NexaMarket satıcısından yeni ürün.", publish: $("#sellerPublish").checked}, true);
        ...
        $("#sellerForm").reset(); $("#sellerCategory").value = "Yaşam"; $("#sellerPrice").value = "249.90"; $("#sellerStock").value = "12"; $("#sellerId").value = String(sellerId); $("#sellerPublish").checked = true;
...
async function loadSellerProducts() {
    const sellerId = currentSellerId(); localStorage.setItem("nexa_seller_id", String(sellerId));
    $("#sellerInventory").innerHTML = `<div class="inventory-loading"><span class="button-spinner"></span>Ürünlerin hazırlanıyor…</div>`;
    try { state.sellerProducts = await api("/api/v1/products/seller", {headers: sellerHeaders(sellerId)}); renderSellerProducts(); }
...
async function changeProductPublication(productId, status, button) {
    ...
    try {
        const updated = await api(`/api/v1/products/${productId}/publication`, {method: "PATCH", headers: sellerHeaders(), body: JSON.stringify({status})});
...
async function deleteSellerProduct(product) {
    await api(`/api/v1/products/${product.id}`, {method: "DELETE", headers: sellerHeaders()});
...
function initEvents() {
    ...
    $("#authForm").addEventListener("submit", submitAuth); $("#sellerForm").addEventListener("submit", submitSeller); $("#sellerId").addEventListener("change", loadSellerProducts); $("#refreshSellerButton").addEventListener("click", loadSellerProducts);
